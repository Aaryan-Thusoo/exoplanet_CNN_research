...
from scripts.helpers.temp_help_name import download_lcs_eb
...
def main() -> None:
    ...
    for start in range(0, len(kic_df), CHUNK_SIZE):
        ...
        for i, kepid in enumerate(kic_df.iloc[start:end]["kepid"]):
            try:
                kic_lcs, depth, deep = download_lcs_eb(kepid, n=LIGHT_CURVE_LENGTH, threshold=shallow_threshold)
    ...
    if failed_kepids:
        ...
        failed_df.to_csv(output_dir / f"failed_eb_downloads.csv", index=False)
