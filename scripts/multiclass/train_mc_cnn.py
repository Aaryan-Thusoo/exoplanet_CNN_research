...
yaml_file = PROJECT_ROOT / "configs"/ "mc_cnn_params.yaml"

def data_loading():
    data_path = PROJECT_ROOT / "data" / "multi_model"
...
def history_saving(history, results_dir: Path) -> None:
    ...
    with open(results_dir / "mc_training_history.json", "w") as file:
...
def results_saving(history, results_dir: Path) -> None:
    ...
    with open(results_dir / "mc_metrics.json", "w") as file:
...
def main() -> None:
    ...
    # Load the params from configs/mc_cnn_params.yaml
    ...
    model.save(model_dir / "multi_model.keras")
