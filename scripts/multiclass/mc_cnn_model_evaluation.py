...
def plot_confusion_matrix(all_true, all_preds):
    ...
    ax.set_title(f"Multiclass CNN Confusion Matrix", fontsize=14, pad=14)
...
def plot_confidence_histogram(
    ...
    plt.title("Multiclass CNN Model Prediction Confidence", fontsize=axis_size)
    plt.xlim(0.5, 1.0)
    plt.tight_layout()

    plt.savefig(RESULTS_DIR / "plots" / "confidence_histogram.png", dpi=200)
    plt.savefig(PAPER_DIR / "mc_confidence_histogram.png", dpi=200)
    plt.close()
...
def plot_correct_incorrect_confidence(
    ...
    plt.xlabel("Prediction Confidence", fontsize=axis_size)
    plt.ylabel("Number of Samples", fontsize=axis_size)
    plt.title("Multiclass CNN Model Prediction Confidence", fontsize=axis_size+2)
    plt.legend()
    plt.tight_layout()
